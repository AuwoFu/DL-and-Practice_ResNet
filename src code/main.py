...
def train(model,train_loader,test_loader,epoch=10,learnRate=0.001):
    ...
    print(f'max test score= {best_score} at epoch{epoch_time+1}')
    return model,best_model,train_record,test_record,best_score


@torch.no_grad()
def test(loadmodel,title='confusion matrix',saveName='./cfm',normalize=False):
    '''
    param:
        loadmodel: path to load model
        saveName: the file name to save confusion matrix figure
    '''
    model=torch.load(loadmodel).to(device)
    model.eval()
    ...
    plot_Confusion_Matrix(y_truth,y_predict,title=title,saveName=saveName,normalize=normalize)
    test_acc=acc/7025*100
...
def main():    
    ...
    tsfm_train = transforms.Compose([
        #transforms.RandomRotation(10,expand=False),# rotate between degree
        transforms.ToTensor()
    ])
    ...
    torch.save(model,f'{modelSaveDir}/{name}')
    torch.save(best_model,f'{modelSaveDir}/best_{name}')
    plt.plot(X,train_record,'-',label=f"train(w/o pretrained)")
    plt.plot(X,test_record,'-',label=f"test(w/o pretrained)")
...
if __name__=="__main__":
    ...
    #modelpath='./22_12-42-25'
    #test('./22_12-42-25/best_pretrain_model_18','pretrain_ResNet18',saveName=f'{modelpath}/cfm_best_pretrain_model_18',normalize=True)